...
RUN git pull
RUN mkdir /app
RUN mv shared/shared cli scripts/system/* requirements/requirements-general.txt /app

ARG MODULE
RUN test -n "$MODULE" || (echo "MODULE not set" && false)
ENV MODULE=$MODULE

RUN mv $MODULE/$MODULE $MODULE/requirements.txt $MODULE/entrypoint.sh /app
