...
from shared.core import (build_fqid, DeletedModelsBehaviour, ModelDoesNotExist, ModelNotDeleted,
    ReadDatabase)
...
class ReaderService:
    ...
    def get(self, request: GetRequest) -> Model:
        with self.database.get_context():
            ...
            if deleted and request.get_deleted_models == DeletedModelsBehaviour.NO_DELETED:
                raise ModelDoesNotExist(request.fqid)
            if not deleted and request.get_deleted_models == DeletedModelsBehaviour.ONLY_DELETED:
                raise ModelNotDeleted(request.fqid)

            if request.position:
                model = self.database.build_model_ignore_deleted(request.fqid, request.position)
                model = self.apply_mapped_fields(model, request.mapped_fields)
            else:
                model = self.database.get(
                    request.fqid, request.mapped_fields
                )
        return model
        ...
        with self.database.get_context():
            ...
            if request.position:
                ...
                if request.get_deleted_models != DeletedModelsBehaviour.ALL_MODELS:
                    fqids = [fqid for fqid in fqids if fqid in deleted_map and deleted_map[fqid] == (request.get_deleted_models == DeletedModelsBehaviour.ONLY_DELETED)]

                result = self.database.build_models_ignore_deleted(fqids, request.position)
                result = self.apply_mapped_fields_multi(result, request.mapped_fields)
            else:
                result = self.database.get_many(fqids,
                    {
    ...
    def aggregate(self, collection, filter, function, field=None, type=None) -> Dict[str, Any]:
        with self.database.get_context():
    ...
    def apply_mapped_fields(
        self, model: Model, mapped_fields: Optional[List[str]]
    ) -> Model:
    ...
    def apply_mapped_fields_multi(
        self, models: Dict[str, Model], mapped_fields: Optional[List[str]]
    ) -> List[Dict[str, Any]]:
        if not mapped_fields or not len(mapped_fields):
            return models
        return {fqid: self.apply_mapped_fields(model, mapped_fields) for fqid, model in models.items()}
