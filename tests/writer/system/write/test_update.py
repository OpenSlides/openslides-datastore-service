...
def test_update(json_client, data, redis_connection, reset_redis_data):
...
def test_update_meta_field(json_client, data, db_cur, redis_connection):
    data["events"][0] = {
        "type": "update",
        "fqid": "a/1",
...
def test_list_update_remove_empty_1(
    ...
    assert_response_code(response, 201)
    assert_model("a/1", {"f": 1}, 2)
    assert_modified_fields(redis_connection, {"a/1": []}, meta_deleted=False)
